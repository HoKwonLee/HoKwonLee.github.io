...
```
**입력**<br>
입력의 첫 줄에는 <u>테스트 케이스의 수 C (C <= 50)</u> 가 주어집니다. 각 테스트 케이스의 첫 줄에는 <u>학생의 수 n (2 <= n <= 10)</u> 과 <u>친구 쌍의 수 m (0 <= m <= n*(n-1)/2)</u> 이 주어집니다. 그 다음 줄에 m 개의 정수 쌍으로 서로 친구인 두 학생의 번호가 주어집니다. 번호는 모두 0 부터 n-1 사이의 정수이고, 같은 쌍은 입력에 두 번 주어지지 않습니다. 학생들의 수는 짝수입니다.
